...
def post_correction_ru(
    ...
    for match in re.finditer(r'\w+', transcription):
        ...
        distance_threshold = (
            distance_thresholds[len(orig_word)]
            if len(orig_word) < len(distance_thresholds)
            else distance_thresholds[-1]
        )
        ...
        if word_freq(lemmatized, lang='ru') > freq_threshold:
            continue
        
        distances = [
            Levenshtein.distance(lemmatized, _preprocess_text(word))
            for word in domain_specific_bag_of_words
        ]
        ...
        closest_word = domain_specific_bag_of_words[cast(int, indices[0])]
