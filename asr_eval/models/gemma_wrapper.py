...
from asr_eval.models.base.interfaces import ContextualTranscriber
...
class Gemma3nWrapper(ContextualTranscriber):
    ...
    def __init__(self, lang: Literal['en', 'ru'] = 'ru', domain_text: str = ''):
        ...
        from transformers.models.gemma3n.processing_gemma3n import (
            Gemma3nProcessor
        )  
        import torch
    ...
    def contextual_transcribe(
        ...
        match self.lang:
            case 'en':
                prompt = (
                    'Transcribe only English speech from'
                    ' audio. Analyze ALL audio, if there is no'
                    ' speech at the beginning - don\'t assume'
                    ' that there is none at all. If there'
                    ' is no speech - return an empty'
                    ' string. Do not add any comments.'
                )
                if self.domain_text:
                    prompt += (
                        f' The speech may contain words'
                        f' from the following text: "{self.domain_text}".'
                    )
                if prev_transcription:
                    prompt += (
                        f' The transcription of the previous'
                        f' part was as follows: "{prev_transcription}".'
                    )
            case 'ru':
                prompt = (
                    'Транскрибируй только русскую речь'
                    ' из аудио. Проанализируй ВСЁ аудио, если в'
                    ' начале нет речи - не думай, что'
                    ' её там вообще нет. Если нет речи - верни'
                    ' пустую строку. Не комментируй.'
                )
                if self.domain_text:
                    prompt += (
                        ' В речи могут встретиться слова'
                        f' из следующего текста: "{self.domain_text}".'
                    )
                if prev_transcription:
                    prompt += (
                        ' Транскрипция предыдущей части'
                        f' была следующей: "{prev_transcription}".'
                    )
        conversation = [{
            "role": "user",
            "content": [
        ...
        # if we remove tokenize=True, we can see the resulting prompt,
        # it will be like this: '<bos><start_of_turn>user\n<audio_soft_token>
        # {prompt}<end_of_turn>\n<start_of_turn>model\n'
