...
from asr_eval.streaming.model import (
    AUDIO_CHUNK_TYPE, OutputChunk, StreamingASR, Signal, TranscriptionChunk
)
...
class VoskStreaming(StreamingASR):
    ...
    def __init__(
        ...
        self._recognizers: dict[ID_TYPE, KaldiRecognizer] = (
            defaultdict(self._make_kaldi_recognizer)
        )
        self._last_was_partial: dict[ID_TYPE, bool] = (
            defaultdict(lambda: False)
        )
        self._transcription_chunk_ref: dict[ID_TYPE, str] = (
            defaultdict(new_uid)
        )
        self.chunk_size = (
            #  * 2 because we receive 2 bytes each frame
            int(sampling_rate * 2 * chunk_length_sec)
    ...
    def _send_transcription_chunk(
        self, id: ID_TYPE, is_final: bool, seconds_processed: float
    ):
        ...
        self.output_buffer.put(OutputChunk(
            data=TranscriptionChunk(
                uid=self._transcription_chunk_ref[id], text=text
            ),
    ...
    def _process_chunk(
        self, id: ID_TYPE, data: AUDIO_CHUNK_TYPE, end_time: float
    ):
