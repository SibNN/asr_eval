...
class LegacyPisetsWrapper(TimedTranscriber):
    ...
    def __init__(
        ...
        use_vad: bool = False,
        whisper_ckpt: str = 'bond005/whisper-large-v3-ru-podlodka',
    ):
        ...
        self.segmenter: Pipeline = (
            initialize_model_for_speech_segmentation('ru')
        )
        if self.use_vad:
            self.voice_activity_detector: Pipeline = (
                initialize_model_for_speech_classification()
            )
        self.asr: Pipeline = initialize_model_for_speech_recognition(
            'ru', model_info=whisper_ckpt
        )
