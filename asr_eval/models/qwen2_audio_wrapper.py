...
class Qwen2AudioWrapper(ContextualTranscriber):
    ...
    def contextual_transcribe(
        ...
        if self.domain_text:
            prompt += (
                ' Words from the following text'
                f' may appear in text: "{self.domain_text}".'
            )
        if prev_transcription:
            prompt += (
                f' The previous transcription was: "{prev_transcription}".'
            )
        conversation = [{
            'role': 'user',
        ...
        prompt = self.processor.apply_chat_template( # type: ignore
            conversation, add_generation_prompt=True, tokenize=False  # type: ignore
        )

        inputs = self.processor( # type: ignore
            text=[prompt], audios=[waveform], return_tensors='pt', padding=True  # type: ignore
