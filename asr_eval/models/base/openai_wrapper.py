...
from asr_eval.utils.server import ServerAsSubprocess
...
class APITranscriber(Transcriber):
    ...
    def __init__(
        ...
        if client == 'run_local_server':
            ...
            client = OpenAI(
                api_key='EMPTY', base_url='http://localhost:8001/v1'
            )
        else:
            self.vllm_proc = None
...
def api_transcribe(
    ...
    response: Transcription = client.audio.transcriptions.create( # pyright: ignore[reportUnknownMemberType]
        file=file,
...
def api_chat_completion(
    ...
    **generate_kwargs: Any,
    ...
    Returns:
        ...
        - Logprobs if :code:`logprobs=True`.
    ...
    # mistral_common.audio.Audio is a container with data, sampling rate,
    # and format; flac is actually a zipped (lossess) wav, should have smaller
    # size than wav
    audio = Audio.from_bytes(
        waveform_to_bytes(waveform, sampling_rate=16_000, format=format)
    )
    ...
    if logprobs:
        assert choice.logprobs is not None # type: ignore
        assert choice.logprobs.content is not None # type: ignore
        returned_logprobs = choice.logprobs.content # type: ignore
    
    return (
        choice.message.content or '',
        choice.finish_reason == 'length',
        returned_logprobs, # type: ignore
    )
