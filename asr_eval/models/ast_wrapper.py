...
class AudioSpectrogramTransformer:
    ...
    def __init__(
        ...
        self.feature_extractor: ASTFeatureExtractor = (
            ASTFeatureExtractor.from_pretrained(model_path) # type: ignore
        )
        self.model: ASTForAudioClassification = (
            ASTForAudioClassification.from_pretrained(model_path).to(device) # type: ignore
        )
        ...
        # min waveform length to pass into feature_extractor and model
        self.min_frames = 240
    
    def predict_on_batch(
        self, waveforms: FLOATS, sampling_rate: int = 16_000
    ) -> FLOATS:   
        import torch
    ...
    def predict_longform(
        ...
        logits = np.concatenate([
            self.predict_on_batch(batch, sampling_rate=sampling_rate)
    ...
    def plot_top_classes(
        self, logits: FLOATS, top_by: Literal['max', 'mean'] = 'max'
    ):
        # logits have shape (n_segments, n_classes)
        reduction = {'max': np.max, 'mean': np.mean}[top_by]
        classes_to_display: list[int] = (
            np.argsort(reduction(logits, axis=0))[:-10:-1].tolist()
        )
