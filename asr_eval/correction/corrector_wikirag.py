...
def _download_nltk_resources():
    resources = [
        'punkt', 'stopwords', 'perluniprops',
        'nonbreaking_prefixes', 'punkt_tab',
    ]
...
class WikipediaTermRetriever(TranscriptionCorrector):
    ...
    def __init__(
        ...
        # )
        if verbose:  
            self.wikipediaapi.log.setLevel(
                level=self.wikipediaapi.logging.DEBUG
            )
            out_hdlr = self.wikipediaapi.logging.StreamHandler(sys.stderr)
            out_hdlr.setFormatter(
                self.wikipediaapi.logging.Formatter('%(asctime)s %(message)s')
            )
        ...
        self.stopwords = set(
            stopwords.words("russian" if lang == 'ru' else 'english') # type: ignore
        )
    ...
    def get_category_articles(
        ...
        for page in (pbar := tqdm(category.categorymembers.values())):
            ...
            if page.ns == self.wikipediaapi.Namespace.MAIN:
                ...
                sub_articles = (
                    self.get_category_articles(page.title.split(":")[1])
                )
    ...
    def text_to_terms(self, text: str) -> list[str]:
        ...
        return [
            ...
            if re.fullmatch(r'[а-яё]{3,}', token)
    ...
    def build_term_index(
        ...
        unique_terms = [
            term
            for term, count in Counter(all_terms).items()
            if count < 20
        ]
    ...
    def find_similar_terms(
        ...
        for term in query_terms:
            ...
            if term in term_index:
                ...
                # Если термина нет в индексе - ищем похожие по написанию и
                # смыслу
                ...
                for index_term in all_index_terms:
                    # Используем комбинацию семантического и строкового
                    # сходства
                    semantic_sim = _vectors_cosine_similarity(
                        term_emb, term_index[index_term]
                    )
