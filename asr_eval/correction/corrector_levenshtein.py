...
from asr_eval.linguistics.linguistics import (
    lemmatize_ru, try_inflect_ru, word_freq
)
...
class CorrectorLevenshtein(TranscriptionCorrector):
    ...
    def get_word_corrections(
        ...
        for match in re.finditer(r'\w+', transcription):
            ...
            closest_word = (
                self.domain_specific_bag_of_words[cast(int, indices[0])]
            )
            closest_word_inflected, inflect_status = (
                try_inflect_ru(closest_word, orig_word)
            )
