...
def draw_timed_transcription(
    ...
    # words
    for block_idx, block in enumerate(transcription.blocks):
        match block:
            case Token():
                ...
                draw_line_with_ticks(
                    ...
                    ax=ax,
                    color='blue' if isinstance(block.value, Wildcard) else 'g',
                    lw=2,
                )
            case MultiVariantBlock():
                for option_idx, option in enumerate(block.options):
                    option_y_pos = (
                        y_pos + y_delta * option_idx / (len(block.options) - 1)
                    )
                    for t in option:
                        draw_line_with_ticks(
                            x1=t.start_time,
                            ...
                            color=(
                                'blue'
                                if isinstance(t.value, Wildcard)
                                else 'g'),
                            lw=2,
                        )
        ...
        match block:
            ...
            case MultiVariantBlock():
                best_option = sorted(
                    block.options,
                    key=lambda option: (
                        len(' '.join(str(t.value) for t in option))
                    )
                )[-1]
    ...
    # bezier connections
    block_spans = [(x.start_time, x.end_time) for x in transcription.blocks]
    joints = (
        [block_spans[0][0] - 0.05]
        + [
            (end1 + start2) / 2
            for (_, end1), (start2, _) in pairwise(block_spans)
        ]
    ...
    for block_idx, block in enumerate(transcription.blocks):
        ...
        match block:
            case Token():
                ...
                if block_idx < len(transcription.blocks) - 1:
                    draw_bezier([
            ...
            case MultiVariantBlock():
                for option_idx, option in enumerate(block.options):
                    ...
                    draw_bezier([
                        (prev_joint if option_idx == 0 \
                            else block.start_time - 0.05, y_pos),
                        (block.start_time, option_y_pos)
                    ], ax=ax, indent=0.05, lw=2)
                    draw_bezier([
                        (block.end_time, option_y_pos),
                        (next_joint if option_idx == 0 \
                            else block.end_time + 0.05, y_pos)
                    ], ax=ax, indent=0.05, lw=2)
                    draw_bezier([
                        (block.start_time, option_y_pos),
                        (block.end_time, option_y_pos)
