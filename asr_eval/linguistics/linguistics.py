...
def get_all_declensions(word: str, keep_capitalized: bool = False) -> set[str]:
    """Gets all the declensions for a Russian word.
    
    Example:
        >>> sorted(get_all_declensions('фейсбук'))
        ['фейсбук', 'фейсбука', 'фейсбуке', 'фейсбуком', 'фейсбуку']
    """
    ...
    normal_form_parsed = _morph.parse(word)[0]
    results: list[str] = []
    for case in ('masc', 'gent', 'datv', 'accs', 'ablt', 'loct'):
        if (inflected := normal_form_parsed.inflect({case})) is not None:
            results.append(inflected.word)
            
    if keep_capitalized and len(word) and word[0].isupper():
        results = [x.capitalize() for x in results]
    ...
    if merge_smaller_than is not None:
        ...
        while True:
            sentences, was_merged = _text_iterative_merge_step(
                sentences,
                merge_smaller_than=merge_smaller_than,
                max_part_size=max_symbols,
            )
            if not was_merged:
                break
...
def split_text_by_space(text: str, max_symbols: int) -> list[str]:
    ...
    for word_match in re.finditer(r'[^\s]+', text):
        if (
            len(parts) > 0
            and word_match.end() - parts[-1][0].start() < max_symbols
        ):
