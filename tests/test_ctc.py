...
from asr_eval.ctc.forced_alignment import forced_alignment_via_recursion, forced_alignment
from asr_eval.utils.types import FLOATS
import asr_eval.ctc.forced_alignment
...
def test_forced_alignment():
    ...
    tokens = [model.vocab.index(x) for x in text]
    log_probs = model.ctc_log_probs([waveform])[0]
    
    # use model.decoding.blank_id as blank token
    
    asr_eval.ctc.forced_alignment.USE_CUSTOM_IMPLEMENTATION = 1
    tokens1, p1, _ = forced_alignment(log_probs, tokens, model.blank_id)
    asr_eval.ctc.forced_alignment.USE_CUSTOM_IMPLEMENTATION = 0
    tokens2, p2, _ = forced_alignment(log_probs, tokens, model.blank_id)
    ...
    for fa in [forced_alignment, forced_alignment_via_recursion]: # type: ignore
