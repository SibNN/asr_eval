...
def model() -> GigaAMShortformCTC:
    return GigaAMShortformCTC('v2', device='cuda')
...
def test_prepare_audio_for_gigaam(model: GigaAMShortformCTC):
    ...
    # load using gigaam
    waveform_torch_from_giaam, length = model._model.prepare_wav(audio_path) # pyright: ignore[reportPrivateUsage]

    # load using librosa
    waveform: FLOATS = librosa.load(audio_path, sr=16_000)[0] # type: ignore
    waveform_torch_from_librosa = (
        torch.tensor(waveform, dtype=model._model._dtype).to(model._model._device).unsqueeze(0) # pyright: ignore[reportPrivateUsage]
    )
    length_from_librosa = (
        torch.tensor([waveform_torch_from_librosa.shape[1]]).to(model._model._device) # pyright: ignore[reportPrivateUsage]
    )
...
def test_giggam(model: GigaAMShortformCTC):
    ...
    assert model._model.transcribe(audio_path1) == expected_text1 # pyright: ignore[reportPrivateUsage]
    assert model._model.transcribe(audio_path2) == expected_text2 # pyright: ignore[reportPrivateUsage]
